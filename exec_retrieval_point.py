...
parser.add_argument('--point_cloud-feat-flie', type=str,
                    default='/home/sse316/heng/cgn/point_feature_pointnet.mat',
...
def main():
    ...
    classifier1 = torch.load(args.model_dir + '/' + args.model + '_baseline_classifiernew_680' + '.pth')
    ...
    sketch_model.load_state_dict(torch.load(args.model_dir + '/' + args.model + '_baseline_sketch_modelnew_680' + '.pth'))
    classifier.load_state_dict(torch.load(args.model_dir + '/' + args.model + '_baseline_classifiernew_680' + '.pth'))
    ...
    with open('example.txt', 'w') as file:
        ...
        file.write('\n')
        while j < len(matrix):
            temp = os.listdir(correct_path + point_cloud_paths[j])[0]
            temp = os.path.splitext(temp)[0]
            file.write(temp)
            file.write(' ')
            file.write(str(matrix[j]))
            file.write('\n')
            print(temp, matrix[j])
            j = j + 1
