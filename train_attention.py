...
from sketch_model_attention import SketchModel
from view_model import MVCNN
from classifier_attention import Classifier
from threed_feat_model_attention2 import POINTCLOUDMODEL
...
parser.add_argument('--point-cloud-batch-size', type=int, default=2)
...
def get_data(sketch_datadir, view_datadir, point_cloud_datadir):
    ...
    sketch_data = SketchDataSet(sketch_datadir, transform=view_transform)
    sketch_dataloaders = DataLoader(sketch_data, batch_size=args.sketch_batch_size, shuffle=True,
                                         num_workers=args.workers)
...
def train(sketch_model, classifier, point_cloud_model, criterion_soft, criterion_am,
    ...
    for batch_idx in range(max(point_cloud_size, sketch_size)):
        ...
        # print(sketch_features.shape,22)
...
def train_mesh(point_cloud_model, classifier, criterion_soft, criterion_am,
          optimizer_model, point_cloud_dataloader, use_gpu):
    point_cloud_model.train()
    classifier.train()
    ...
    for batch_idx in range(point_cloud_size):
        mesh = next(point_cloud_dataloader_iter)
        mesh_data, mesh_labels = mesh

        if use_gpu:
            mesh_data, mesh_labels = mesh_data.cuda(), mesh_labels.cuda()

        mesh_features = point_cloud_model.forward(mesh_data)

        concat_feature = mesh_features
        concat_labels = mesh_labels
...
def main():
    ...
    # sketch_model.load_state_dict(torch.load(args.model_dir + '/' + args.model + '_baseline_sketch_modelnew_80' + '.pth'))
    # classifier.load_state_dict(torch.load(args.model_dir + '/' + args.model + '_baseline_classifiernew_80' + '.pth'))
    # point_cloud_model.load_state_dict(torch.load(args.model_dir + '/' + args.model + '_baseline_point_cloud_modelnew_80' + '.pth'))
    ...
    for epoch in range(args.max_epoch):
        ...
        train(sketch_model, classifier, point_cloud_model, criterion_soft, criterion_am,
              optimizer_model, sketch_trainloader, view_trainloader, point_cloud_trainloader, use_gpu)
        # return
        ...
        if epoch > 0 and val_acc > best_acc:
            best_acc = val_acc
            torch.save(sketch_model.state_dict(),
                       args.model_dir + '/' + args.model + '_best_baseline_sketch_modelnew' + '_' + str(epoch) + '.pth')
            # torch.save(view_model.state_dict(),
            #            args.model_dir + '/' + args.model + '_best_baseline_view_modelnew' + '.pth')
            torch.save(classifier.state_dict(),
                       args.model_dir + '/' + args.model + '_best_baseline_classifiernew'+ '_' + str(epoch) + '.pth')
            torch.save(point_cloud_model.state_dict(),
...
def main_sketch():
    ...
    sketch_trainloader, val_sketch_dataloader, view_trainloader, point_cloud_trainloader = get_data(args.sketch_datadir,
                                                                                                    args.view_datadir,
                                                                                                    args.point_cloud_datadir)
...
def main_mesh():
    ...
    criterion_am = AMSoftMaxLoss()
    criterion_soft = nn.CrossEntropyLoss()
    optimizer_model = torch.optim.SGD([{"params": point_cloud_model.parameters()},
                                       {"params": classifier.parameters(), "lr": args.lr_model * 10}],
                                      lr=args.lr_model, momentum=0.9, weight_decay=1e-4)
    ...
    for epoch in range(args.max_epoch):
        ...
        avg_acc = train_mesh(point_cloud_model, classifier, criterion_soft, criterion_am,
                               optimizer_model, point_cloud_trainloader, use_gpu)

        if epoch > 60 and epoch % args.save_model_freq == 0:
            torch.save(point_cloud_model.state_dict(),
                       args.model_dir + '/' + args.model + '_baseline_mesh_model' + '_' + str(epoch) + '.pth')
            torch.save(classifier.state_dict(),
                       args.model_dir + '/' + args.model + '_baseline_classifier' + '_' + str(epoch) + '.pth')
