...
parser.add_argument('--train-datadir', type=str, default='/home/sse316/heng/cgn/datasets/shrec_13')
parser.add_argument('--test-datadir', type=str, default='/home/sse316/heng/cgn/datasets/shrec_13')
# parser.add_argument('--train-datadir', type=str, default='E:/3d_retrieval/Dataset/vis/view')
# parser.add_argument('--test-datadir', type=str, default='E:/3d_retrieval/Dataset/vis/view')
...
# parser.add_argument('--model-dir', type=str,choices=['/data/david/sketch_uncertainty/RES','resnet50'], default='./alexnet_13_best')
parser.add_argument('--model-dir', type=str,default='/home/sse316/heng/cgn/trained/resnet50')
...
parser.add_argument('--test-feat-dir', type=str, default='point_feature_pointnet_connect_withweight_4096_jz24096_resnet50.mat')
...
def get_test_data(traindir):
    ...
    point_cloud_data = PointCloudDataSet(traindir, transform=view_transform)
    ...
    dataloaders = DataLoader(point_cloud_data, batch_size=args.batch_size, shuffle=False, num_workers=args.workers)
    return dataloaders
...
def main():
    ...
    classifier = Classifier(35,args.cnn_feat_dim,args.num_classes)
    if use_gpu:
        point_cloud_model = nn.DataParallel(point_cloud_model).cuda()
        classifier = nn.DataParallel(classifier).cuda()
    # Load model
    # print(args.model_dir + '/' + args.model + '_best_baseline_point_cloud_modelnew_84'  + '.pth')
    point_cloud_model.load_state_dict(
        torch.load(args.model_dir + '/' + args.model + '_best_baseline_point_cloud_modelnew'  + '.pth'))
    classifier.load_state_dict(
        torch.load(args.model_dir + '/' + args.model + '_best_baseline_classifiernew'  + '.pth'))
    ...
    point_cloud_feature = np.zeros((num_samplses, args.feat_dim))
    point_cloud_labels = np.zeros((num_samplses, 1))
    ...
    for batch_idx, (data, labels,path) in enumerate(trainloader):
        print(path)
        # path=path[0].split('/')[-3]+'/' + path[0].split('/')[-2]+'/'+path[0].split('/')[-1]
        ...
        output = point_cloud_features[0]

        # data = data.permute(0, 2, 1)
        # data = data.float().cuda()
        # output = point_cloud_model.forward(data)
        # output = output[0]

        # linear_layer = nn.Linear(512, 4096)
        ...
        if args.uncer:
            ...
            x2, logits = classifier.forward(output,1)
        ...
        point_cloud_labels[batch_idx] = labels_numpy
        predict_labels[batch_idx] = predicts_numpy
        point_cloud_feature[batch_idx] = outputs_numpy
        paths.append(path)
    ...
    point_cloud_feature_data = {'point_cloud_feature': point_cloud_feature, 'point_cloud_labels': point_cloud_labels,
                         'predict_label':predict_labels,'point_cloud_paths':paths}
    torch.save(point_cloud_feature_data,args.test_feat_dir)
