...
parser.add_argument('--point-cloud-datadir', type=str, default='.\datasets\shrec_13\point_clouds')
...
# optimization
parser.add_argument('--sketch-batch-size', type=int, default=1)
parser.add_argument('--view-batch-size', type=int, default=1)
parser.add_argument('--point-cloud-batch-size', type=int, default=1)
...
def get_data(sketch_datadir, view_datadir, point_cloud_datadir):
    ...
    point_cloud_data = PointCloudDataSet(point_cloud_datadir)
...
def read_off_file(file_path):
    with open(file_path, 'r') as file:
        ...
        # 如果点数少于，将其扩充
        if num_points < 35:
            additional_points = [[0.0, 0.0, 0.0] for _ in range(35 - num_points)]
            points += additional_points

    return np.array(points)

def point(dir):
    point_cloud = read_off_file(dir)
    # 确定批次大小和点的数量
...
def train(sketch_model, view_model, classifier, point_cloud_model, criterion_soft, criterion_am,
    ...
    for batch_idx in range(max(point_cloud_size, sketch_size)):
        ...
        # print('111', sketch_data.shape, sketch_labels)
        # print('111', point_cloud_data, point_cloud_labels)
        # view_data = view[0]
        # view_labels = view[1]
        ...
        sketch_features = sketch_model.forward(sketch_data)
        point_cloud_features = point_cloud_model.forward(point_cloud_data)
    ...
    for epoch in range(args.max_epoch):
        ...
        if 1:
            ...
            torch.save(point_cloud_model.state_dict(),
                       args.model_dir + '/' + args.model + '_baseline_point_cloud_model' + '_' + str(epoch) + '.pth')
        if epoch > 0 and val_acc > best_acc:
            ...
            torch.save(point_cloud_model.state_dict(),
                       args.model_dir + '/' + args.model + '_best_baseline_point_cloud_model' + '_' + str(epoch) + '.pth')
