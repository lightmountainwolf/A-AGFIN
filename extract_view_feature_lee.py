...
parser.add_argument('--test-datadir', type=str, default='/home/sse316/heng/cgn/datasets/shrec_13/multy_view')
...
parser.add_argument('--model-dir', type=str,default='/home/sse316/heng/cgn/trained/multy_view')
...
parser.add_argument('--test-feat-dir', type=str, default='/home/sse316/heng/cgn/alex_L2_test_view_feature_multy_view.mat')
...
def main():
    ...
    view_model.load_state_dict(
        torch.load(args.model_dir + '/' + args.model + '_best_baseline_view_model'  + '.pth'))
    classifier.load_state_dict(
        torch.load(args.model_dir + '/' + args.model + '_best_baseline_classifier'  + '.pth'))
    ...
    for batch_idx, (data, labels,path) in enumerate(trainloader):
        ...
        print(batch_idx,labels_numpy,predicts_numpy,path)

        np.set_printoptions(threshold=np.inf)

        with open('/home/sse316/heng/cgn/extract_features/alex_L2_test_view_feature_lee.txt', 'w') as f:
            f.write(str(labels_numpy))
            f.write(str(predicts_numpy))
            f.write(str(path))
    ...
    view_feature_data = {'view_feature': view_feature, 'view_labels': view_labels,
                         'predict_label':predict_labels,'view_paths':paths}
    torch.save(view_feature_data,args.test_feat_dir)


    # torch.save(view_feature_data, '/home/sse316/heng/cgn/extract_features/alex_L2_test_view_feature_lee.txt')
